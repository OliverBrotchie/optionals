...
The standard practice of returning `null`/`undefined` when no value other can be returned, leaves something to be desired. There is no way to tell where in the code said value came from. Rust's implementation of error handling (e.g. bubbling) also has many benefits that cannot be expressed in the normal `try`/`catch` JS pattern.
